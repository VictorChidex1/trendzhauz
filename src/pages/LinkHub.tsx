  ...
  const getIcon = (type: string, isDark: boolean) => {
    switch (type.toLowerCase()) {
      case "spotify":
        return (
          <svg viewBox="0 0 24 24" fill="#1DB954" className="w-6 h-6">
            <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.24 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15.001 10.62 18.72 12.9c.36.181.54.78.241 1.14zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.6.18-1.2.72-1.38 4.26-1.32 11.28-1.02 15.721 1.621.54.3.72.96.42 1.5-.3.54-.96.72-1.56.36z" />
          </svg>
        );
      case "apple":
        return (
          <svg viewBox="0 0 24 24" fill="#FA243C" className="w-6 h-6">
            <path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-1.9.04-3.664 1.11-4.632 2.787-1.952 3.39-.497 8.423 1.405 11.164.928 1.34 2.022 2.84 3.473 2.791 1.378-.04 1.91-.877 3.477-.877 1.563 0 2.05.836 3.483.877 1.488.04 2.417-1.3 3.33-2.628 1.059-1.546 1.495-3.045 1.517-3.12-.03-.016-2.92-1.12-2.946-4.464-.025-2.79 2.277-4.137 2.385-4.197-1.306-1.905-3.328-2.164-4.043-2.222-1.782-.163-3.528 1.04-4.489 1.04v-.113zm1.5-3.11c.789-.955 1.32-2.283 1.176-3.6-.11.05-.173.08-.236.104-1.264.475-2.656 1.258-3.535 2.275-.767.893-1.404 2.247-1.229 3.551 1.407.108 2.873-.64 3.824-2.33z" />
          </svg>
        );
      case "audiomack":
        return (
      ...
      case "mail":
        return (
          <Mail
            className={`w-6 h-6 ${isDark ? "text-white" : "text-zinc-900"}`}
          />
        );
      default:
        return (
    ...
    >
      ...
      <div className="z-10 w-full max-w-md flex flex-col items-center">
        ...
        <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full mb-6 relative group">
          ...
          <div
            className={`absolute inset-0 rounded-full blur-xl scale-110 transition-opacity duration-500 -z-10 ${isDarkMode ? "bg-white/10 opacity-50" : "bg-black/5 opacity-50"}`}
          />
        </div>

        <h1
          className={`text-2xl sm:text-3xl font-black uppercase tracking-tight mb-1 ${isDarkMode ? "text-white" : "text-zinc-900"}`}
        >
          TRENDZHAUZ
        </h1>
        <p
          className={`text-sm sm:text-base mb-10 text-center max-w-[280px] font-medium ${isDarkMode ? "text-zinc-400" : "text-zinc-600"}`}
        >
          Listen to our playlists across different platforms, view bookings, and
          connect us.
        </p>
          ...
          <div className="w-full flex flex-col gap-3.5">
            ...
            >
              <div className="flex items-center gap-4 z-10 relative">
                ...
                >
                  <Globe
                    className={`w-6 h-6 ${isDarkMode ? "text-white" : "text-zinc-900"}`}
                  />
                </div>
                <span
              ...
              <div
                className={`z-10 relative text-[11px] font-bold uppercase tracking-wider transition-colors ${
                  isDarkMode
        ...
        <div className="mt-16 text-center">
          <a
            href="/"
            className={`text-[10px] uppercase tracking-[0.2em] font-bold hover:text-orange-500 transition-colors ${
